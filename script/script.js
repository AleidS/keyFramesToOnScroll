                    ...
                    animationNames.forEach((animationName) => {
                        ...
                        if (animationName != undefined && !animationName.includes('exclude') && !animationName.includes('Exclude') && animationName == keyFrameName) {
                            ...
                            target.each((i, element1) => {
                                ...
                                // Made an option to disable this script, but doesn't work very well.
                                else {

                                    element1.style.setProperty('animation-duration', '3s', 'important');
...
function switchScript() {
    ...
    if (generateAnimations) {
        $('#switchLabel').html('On Scroll Animations')
    }
    else {
        $('#switchLabel').html('keyFrame Animations (with color glitch, idk why)')
    }
    iterateRules(generateAnimations)
}
